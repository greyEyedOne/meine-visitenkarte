window.klaroConfig = {
  ...
  additionalClass: 'klaro-container',
  ...
  services: [
      ...
      callback: function (consent, service) {
        ...
        if (typeof window !== 'undefined' && service.name === 'vercel-analytics') {
          ...
          if (consent === true) {
            ...
            // Best-effort: remove a small cookie name if present
            try {
              document.cookie = 'vercel-analytics=; Max-Age=0; path=/'
            } catch (e) {
              // ignore
            }
          }
    ...
    },
    {
      ...
      callback: function (consent, service) {
        console.log(`User consent for service ${service.name}: consent=${consent}`)
        if (typeof window !== 'undefined') {
          const selector = 'script[data-analytics="vercel-speed"]'
          if (consent === true) {
            if (!document.querySelector(selector)) {
              const s = document.createElement('script')
            ...
            if (existing) existing.remove()
            try {
              document.cookie = 'vercel-speed-insights=; Max-Age=0; path=/'
            } catch (e) {
              // ignore
            }
          }
    ...
    /*   
    !!!! a reminder to move GFonts to the server
    {
            name: 'googleFonts',
            title: 'Google Fonts',
            purposes: ['styling'],
